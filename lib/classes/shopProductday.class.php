...
class shopProductday {
    ...
    public static function getDomainsSettings() {
        ...
        $cache = new waSerializeCache('shopProductdayPlugin');
        ...
        if ($cache && $cache->isCached()) {
            ...
            $app_settings_model->get(shopProductdayPlugin::$plugin_id, 'domains_settings');
            $domains_settings = json_decode($app_settings_model->get(shopProductdayPlugin::$plugin_id, 'domains_settings'), true);
            ...
            foreach ($domains_routes as $domain => $routes) {
                foreach ($routes as $route) {
                    ...
                    if (empty($domains_settings[$domain_route])) {
                        $domains_settings[$domain_route] = shopProductdayPlugin::$default_settings;
                    }
                    foreach (shopProductdayPlugin::$default_settings['templates'] as $tpl_name => $tpl) {
    ...
    public static function saveDomainsSettings($domains_settings) {
        ...
        foreach ($domains_routes as $domain => $routes) {
            foreach ($routes as $route) {
                ...
                foreach (shopProductdayPlugin::$default_settings['templates'] as $id => $template) {
        ...
        $app_settings_model->set(shopProductdayPlugin::$plugin_id, 'domains_settings', json_encode($domains_settings));
        $cache = new waSerializeCache('shopProductdayPlugin');
        if ($cache && $cache->isCached()) {
            $cache->delete();
        }
